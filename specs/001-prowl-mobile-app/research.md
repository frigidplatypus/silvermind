# Research: Prowl Mobile App

**Date**: 2026-06-18
**Feature**: [spec.md](./spec.md)
**Plan**: [plan.md](./plan.md)

This document records the key technical decisions and best practices used to design Prowl. Each section captures: the **Decision**, the **Rationale**, and **Alternatives considered**.
...
**Decision**: sbtask `serve` listens on a fixed port (e.g., 7433 — a non-privileged high port). Prowl's API client always calls `http://127.0.0.1:7433/`. The port is documented in the contracts as a constant.
...
- Configuring spaces via the CLI is a power-user concern, not a Prowl concern.
...
**Decision**: Prowl is a thin client. All task and space data is owned by sbtask. Prowl fetches and renders it; sbtask persists it. Prowl does not maintain a local database or sync engine.

**Rationale**:
- Spec describes Prowl as a UI wrapper around sbtask.
