<script lang="ts">
  ...
  let extraAttrs = $state<Record<string, string>>({});
  let extraAttrCounter = 0;
  let alerts = $state<string[]>([]);
  ...
  function handleTagKeydown(e: KeyboardEvent) {
    if (e.key === 'ArrowDown') {
      e.preventDefault();
      tagSelectedIndex = Math.min(tagSelectedIndex + 1, tagFiltered.length - 1);
...
{#snippet formContent()}
  ...
  <textarea
    ...
    bind:value={text}
    rows={6}
    placeholder="Task text (markdown)…"
  ></textarea>
      ...
      <div class="chip-row tag-chips">
        {#each tags as tag}
          <span class="chip"
            ...
            {tag}
            <button class="chip-remove" onclick={() => removeTag(tag)} aria-label="Remove {tag}"
              >×</button
            ></span
          >
        {/each}
      ...
      {#if tagFocused && tagFiltered.length > 0}
        <ul class="ac-dropdown" role="listbox">
          {#each tagFiltered as item, i}
            <li
              class="ac-item"
              class:selected={i === tagSelectedIndex}
              role="option"
              aria-selected={i === tagSelectedIndex}
              onpointerdown={(e) => {
                e.preventDefault();
  ...
  <Autocomplete
    ...
    onselect={(name) => (parent = name)}
  />
  {#if parent}
    <div class="chip-row">
      <span class="chip"
        ><Icon name="corner-right-up" size="0.75rem" />
        {parent}
        <button class="chip-remove" onclick={() => (parent = '')} aria-label="Remove parent"
          >×</button
        ></span
      >
    </div>
  ...
  {#if alerts.length > 0}
    <div class="chip-row alert-chips">
      {#each alerts as alert}
        <span class="chip alert-chip"
          ><Icon name="bell" size="1rem" />
          {alert}
          <button
            class="chip-remove"
            onclick={() => (alerts = alerts.filter((a) => a !== alert))}
  ...
  <div class="alert-input-row">
    ...
    <button
      class="alert-add-btn"
      disabled={!newAlertDate || !newAlertTime}
      onclick={() => {
  ...
  <div class="field-label">Custom Attributes</div>
  <div class="extra-attrs">
    {#each Object.keys(extraAttrs) as key}
      <div class="extra-attr-row">
        <input
          type="text"
          class="field extra-key"
          value={key}
          placeholder="key"
          oninput={(e) => {
            const v = (e.target as HTMLInputElement).value;
            const val = extraAttrs[key];
            delete extraAttrs[key];
            extraAttrs[v] = val;
        ...
        <input
          type="text"
          class="field extra-val"
          value={extraAttrs[key]}
          placeholder="value"
          oninput={(e) => {
            extraAttrs[key] = (e.target as HTMLInputElement).value;
            extraAttrs = { ...extraAttrs };
          }}
        />
        <button
    ...
    <button
      class="extra-add"
      onclick={() => {
        extraAttrs[`__new_${++extraAttrCounter}`] = '';
        extraAttrs = { ...extraAttrs };
  ...
  <div class="meta-row">
    <span class="meta-label">Page</span><span class="meta-value">{task.page}</span>
  </div>
  <div class="meta-row">
  ...
  <div class="delete-section">
    ...
    {:else}
      <button class="delete-trigger" onclick={() => (showDeleteConfirm = true)}
        ><Icon name="trash-2" /> Delete task</button
      >
    {/if}
  </div>
{/snippet}
