...
export function getTheme(): Mode { return currentMode; }
export function getAccent(): AccentPreset { return currentAccent; }
...
export function setTheme(mode: Mode): void {
  currentMode = mode;
  try { localStorage.setItem('silvermind-theme', mode); } catch { /* noop */ }
  apply();
}

export function setAccent(accent: AccentPreset): void {
  currentAccent = accent;
  try { localStorage.setItem('silvermind-accent', accent); } catch { /* noop */ }
  apply();
}
