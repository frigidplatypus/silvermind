<script lang="ts">
  ...
  function handleToggle(task: Task) {
    return onToggleDone ? onToggleDone(task) : toggleTaskDone(task, () => loadToday());
  }
...
<div class="today-page">
  ...
  {:else}
    ...
    {/if}
    <section>
      <h2 class="heading overdue">Overdue</h2>
      {#each overdue as t (tid(t))}<TaskRow
          task={t}
          id={tid(t)}
    ...
    <section>
      <h2 class="heading">Due Today</h2>
      {#each dueToday as t (tid(t))}<TaskRow
          task={t}
          id={tid(t)}
          onclick={() => handleTaskTap(t)}
          onToggleDone={handleToggle}
    ...
    <section>
      <h2 class="heading">Deferred Today</h2>
      {#each deferredToday as t (tid(t))}<TaskRow
          task={t}
          id={tid(t)}
          onclick={() => handleTaskTap(t)}
          onToggleDone={handleToggle}
...
<style>
  ...
  .heading {
    padding: var(--space-3) var(--space-4) 0.375rem;
